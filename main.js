...
const starts = async (zeroyt7 = new WAConnection()) => {
    ...
    fs.existsSync('./MBP_17.json') && zeroyt7.loadAuthInfo('./MBP_17.json')
    ...
    await zeroyt7.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./MBP_17.json', JSON.stringify(zeroyt7.base64EncodedAuthInfo(), null, '\t'))

    zeroyt7.on('chat-update', async (message) => {
        require('./ItsBayu.js')(zeroyt7, message, _welkom)
    })
...
function nocache(module, cb = () => { }) {
    console.log('Module', `'${module}'`, 'sekarang sedang diawasi Lord Bayu untuk perubahan')
